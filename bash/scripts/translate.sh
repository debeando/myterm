...
# adjust to taste
declare -r FIRST_LANG=es      #target language for request in LATIN_LANG    NOT in A-z latin alphabet
...
#echo $grespond | grep -o '\[.*\]'
    ...
    if [[ $r_words_count -le 2 && ${#request} -gt 1 && $source == en && (( ! $fl_raw || $trans )) ]]; then #dictionary
...
-e 's/^\([0-9]*\):.*\"EXAMPLE\">\([^<]*\).*/**\1 \2/' | grep -o "^\*\*[0-9].*" | sed 's/^\*\*//' ) # doubleh2+exmaple, h2+exmaple, doubleh2+sentence, h2+sentence, only example
      if [[ $phras ]]; then   #phrases
        ...
        phrases_2=$(echo "$macmill" | sed -e 's/<a[^>]*>//g' -e 's/<\/a>//g' | sed 's/<.\?span[^>]*>//g' | grep -o 'div class="SENSE".*End of DIV SENSE--' | sed 's/End of DIV SENSE--/\n/g' | grep -n . | sed 's/^.:/&\n/' | sed 's/End of DIV EXAMPLES/\n/g' | sed '/^[0-9]:/{h;d;};G; s/^\(.*\)\n\([0-9]:\)/\2 \1/' | grep "<strong>" | sed 's/^\([0-9]\).*<strong>\([^<]*\)<.*class=\"EXAMPLE\">\([^<]*\)<.*/\1 \'$C_BRIGHT_RAW'\2'$C_NORMAL'. \3/' | sed 's/^\([0-9]\).*<strong>\([^<]*\)<\/strong>\([^<]*\).*/\1 \'$C_BRIGHT_RAW'\2'$C_NORMAL'. \3/')
        if [[ $phrases_2 ]];then
      ...
      if [[ ${#request} -gt 2 || $r_words_count -le 2 ]] ; then
        ...
        [[ ! -e "$cachefile".mp3 ]] &&
          if [[ $sound == 1 ]]; then
            eng_sound_download
          elif [[ $SOUND_DOWNLOAD_AWS == 1 ]];then
